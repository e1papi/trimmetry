...
SPEED MODE
   ...
   ``MOMENTAL`` при достяжении "разрыва" между первой и последней точкой происходит резкий скачок.
   ``DEFAULT`` при достяжении "разрыва" между первой и последней точкой скорость движения сохраняется той же, как если бы точки были соединены.
